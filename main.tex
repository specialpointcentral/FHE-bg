...
\begin{frame}{4.1 TFHE Gate Operations}
  \begin{description}
    ...
    \item[NOT gate] Flip the phase to encode $1 - m$ using $(1 - m)\mu = \mu - m\mu$:
    $$\text{NOT}(C) = \bigl(\mathbf{a},\; \mu - c_0\bigr) \bmod q,$$
    with $\mu$ serving as the same scaling constant used earlier.
  ...
  \end{description}
  \note{“This slide shows how TFHE handles Boolean gates. Each encrypted bit is an LWE-style pair $(\mathbf{a}, c_0)$ with $c_0 = \langle \mathbf{a}, s\rangle + \mu m + e$. NOT is trivial: since $(1 - m)\mu = \mu - m\mu$, we can flip the phase so $\text{NOT}(C) = (\mathbf{a}, \mu - c_0) \bmod q$. XOR is performed by ciphertext addition: $\text{XOR}(C_0, C_1) = C_0 + C_1 \pmod{q}$. The difficult gates are AND or NAND because multiplying the encoded messages triggers $(m_0 \mu)(m_1 \mu) = (m_0 m_1)\mu^2$, squaring the scale and rapidly blowing up noise. TFHE addresses this by bootstrapping immediately after such gates, which keeps the noise budget fresh and enables deep Boolean circuits.”}
\end{frame}
