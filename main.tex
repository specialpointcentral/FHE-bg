...
\begin{frame}{2.3 LWE and RLWE}
  ...
  \end{itemize}
  \note{Introduce why we need LWE/RLWE: they add controlled random noise to hide plaintexts while preserving the ring/polynomial structure needed for computation, with security reducible to GapSVP even against quantum adversaries. Briefly define LWE via noisy linear equations $b = A\seckey{s} + e \pmod{q}$ and RLWE via polynomial samples $b(x) = a(x)\seckey{s(x)}+e(x) \pmod{q}$ inside $R_q$. Emphasize that RLWE inherits LWE hardness yet aligns with the polynomial ring container used in FHE, enabling efficient and analyzable homomorphic schemes.}
\end{frame}
...
\begin{frame}{4.1 TFHE Gate Operations}
  ...
  \end{description}
  \note{“Each TFHE ciphertext encrypts a bit $\plaintext{m} \in \{0,1\}$ as $(A, c_1)$ with $c_1 = A s + \mu \plaintext{m} + e$. For NOT we flip the phase: $(1 - \plaintext{m})\mu = \mu - \plaintext{m}\mu$, so the refreshed component becomes $c_1' = A s + \mu (1 - \plaintext{m}) + e$ and the ciphertext is $( -A,\; \mu - c_1 ) \bmod q$. XOR is simply ciphertext addition because the arithmetic is linear, so $\text{XOR}(c, c') = c + c' \pmod{q}$. AND and NAND are harder: multiplying ciphertexts corresponds to $(\mu \plaintext{m}_0)(\mu \plaintext{m}_1) = \mu^2 \plaintext{m}_0 \plaintext{m}_1$, which squares the scale and noise. TFHE combats that by bootstrapping right after multiplicative gates, keeping the noise budget low enough for deep Boolean circuits.”}
\end{frame}
...
\begin{frame}{4.2 TFHE Programmable Bootstrapping (PBS)}
  ...
  \end{description}
  \note{“PBS starts from the phase representation: TLWE ciphertexts encode a phase near either $0$ or $\mu$ depending on the bit. The goal is to extract that phase homomorphically, evaluate the desired Boolean function, and re-encrypt a fresh ciphertext so evaluation can continue. Conceptually it’s a decode–evaluate–re-encrypt pipeline running entirely under encryption.”}
\end{frame}
...
\begin{frame}{4.2 TFHE Programmable Bootstrapping (PBS)}
  ...
  \end{description}
  \note{“The first PBS step is phase reconstruction. We’d like $\phi = c_1 - A\seckey{s}$, but naively subtracting $A\seckey{s}$ would reveal the secret key. Instead we work with $\Enc(\phi) = \Enc(c_1 - A\seckey{s})$, which means we only need $\Enc(A\seckey{s})$. Because $A\seckey{s} = \sum_i a_i s_i$, we can publish GLWE encryptions of each secret coefficient and homomorphically sum $\sum_i a_i \Enc_{\mathrm{GLWE}}(s_i) = \Enc(A\seckey{s})$. This collection is the bootstrapping key, letting PBS ‘decrypt itself’ just enough to refresh the ciphertext while keeping \seckey{s} hidden.”}
\end{frame}
...
\begin{frame}{4.2 TFHE Programmable Bootstrapping (PBS)}
  ...
  \end{description}
  \note{“Once we hold $\Enc(\phi) = \Enc(c_1 - A s)$, PBS evaluates the requested Boolean function via a programmable LUT. Run gadget decomposition to obtain $(w_0, \ldots, w_k)$, keep the encrypted outputs $\mathrm{LUT}_f$, and compute the external product $\mathrm{LUT}_f \bigstar (w_0, \ldots, w_k) = \sum_i w_i \cdot \mathrm{LUT}_f^{(i)}$. The $w_i$ are encrypted selection weights while $\mathrm{LUT}_f^{(i)}$ stores the encrypted function value. This finishes the ciphertext transformation: because we can prebuild $\mathrm{LUT}_f$ for any Boolean function, PBS is truly “programmable bootstrapping.”}
\end{frame}
